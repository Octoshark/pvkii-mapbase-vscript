...
// @NMRiH - Felis: Particle effect dispatch by attachment
static void ScriptDispatchParticleEffectByAttachment( const char *pszParticleName, int iAttachType, const HSCRIPT hEntity, const char *pszAttachmentName, const bool bResetAllParticlesOnEntity )
{
	if ( iAttachType < 0 || iAttachType >= MAX_PATTACH_TYPES )
		return;

	const ParticleAttachment_t particleAttachment = (ParticleAttachment_t)iAttachType;
...
void RegisterSharedScriptFunctions()
{
	...
	// @NMRiH - Felis
	ScriptRegisterFunctionNamed( g_pScriptVM, ScriptDispatchParticleEffectByAttachment, "DispatchParticleEffectByAttachment", "Dispatches one-off particle system by attaching to an entity. " 
		"Params: <particle name>, <attach type>, <target entity>, <attachment name>, <reset all particles on entity?>");
